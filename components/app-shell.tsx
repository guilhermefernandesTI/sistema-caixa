...
import { BarChart3, Boxes, ChevronDown, LayoutDashboard, LogOut, Menu, PaintBucket, ReceiptText, Settings2, WalletCards, X } from "lucide-react";
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[var(--brand-bg)] text-[var(--brand-text)]">
      <aside className="fixed inset-y-0 left-0 z-30 flex w-64 flex-col bg-[var(--brand-sidebar)] px-5 py-6 text-white transition-transform lg:translate-x-0" style={{ transform: open ? "translateX(0)" : "translateX(-100%)" }}>
      ...
      <div className="lg:pl-64">
        <header className="sticky top-0 z-10 flex h-[72px] items-center justify-between border-b border-[var(--brand-secondary)] bg-[var(--brand-bg)] px-5 backdrop-blur-md sm:px-8">
          ...
          <div className="ml-auto flex items-center gap-3">
            ...
            <div className="grid h-9 w-9 place-items-center rounded-full bg-[var(--brand-accent)] text-xs font-bold text-[var(--brand-sidebar)]">{user.displayName.slice(0,2).toUpperCase()}</div>
          </div>
        </header>
